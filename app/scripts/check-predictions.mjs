        ...
        : p.match_id <= 100
          ...
          : p.match_id <= 102
            ...
            : p.match_id === 103
              ...
              : "Final";
  const tie =
    p.home_goals === p.away_goals ? " *** TIE - winner_id:" + p.winner_id : "";
  console.log(
    `  match ${p.match_id} ${stage} ${p.home_goals}-${p.away_goals}${tie}`,
  );
});
...
if (!e3 && allKnockoutTies) {
  const knockoutTiesNoWinner = allKnockoutTies.filter(
