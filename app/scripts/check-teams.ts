  ...
  try {
    ...
    if (resp.ok) {
      ...
      for (const id of tiedIds) {
        ...
        if (m) {
          console.log(
            `  match ${id}: ${m.homeTeam?.name || "TBD"} (id:${m.homeTeam?.id}) vs ${m.awayTeam?.name || "TBD"} (id:${m.awayTeam?.id}) [${m.stage}]`,
          );
        } else {
      ...
      groupMatches.forEach((m: any) => {
        if (m.homeTeam?.id)
          teams.set(m.homeTeam.id, m.homeTeam.name || m.homeTeam.shortName);
        if (m.awayTeam?.id)
          teams.set(m.awayTeam.id, m.awayTeam.name || m.awayTeam.shortName);
      });
