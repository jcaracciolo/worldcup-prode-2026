...
export async function POST(request: NextRequest) {
  try {
    ...
    // Filter by stage
    const targetMatches = stage === 'group'
    ...
    for (const match of targetMatches) {
      ...
      const updatedMatch = {
        ...
        score: {
          ...
          halfTime: { home: Math.floor(homeGoals / 2), away: Math.floor(awayGoals / 2) },
        },
      }
    ...
    return NextResponse.json({ success: true, matchesUpdated: targetMatches.length })
  } catch (error) {
