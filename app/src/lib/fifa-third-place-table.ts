/**
 * Official FIFA World Cup 2026 Third-Place Team Pairings Table
 *
 * This table contains all 495 possible combinations of which 8 third-place teams
 * qualify from the 12 groups (C(12,8) = 495), and the predetermined assignment
 * of each to the 8 R32 matches against group winners.
 *
 * Source: FIFA World Cup 2026 Regulations, Annex C (also published on Wikipedia)
 *
 * Key format: The 8 qualifying groups sorted alphabetically (e.g., "ABCDEFGH")
 * Value format: Array of 8 group letters for matches [74, 77, 79, 80, 81, 82, 85, 87]
 *
 * These matches are:
 *   - Match 74: Winner Group E vs 3rd place team
 *   - Match 77: Winner Group I vs 3rd place team
 *   - Match 79: Winner Group A vs 3rd place team
 *   - Match 80: Winner Group L vs 3rd place team
 *   - Match 81: Winner Group D vs 3rd place team
 *   - Match 82: Winner Group G vs 3rd place team
 *   - Match 85: Winner Group B vs 3rd place team
 *   - Match 87: Winner Group K vs 3rd place team
 */

// Match numbers in order of the assignment array
export const THIRD_PLACE_MATCH_ORDER = [
  74, 77, 79, 80, 81, 82, 85, 87,
] as const;

// The full FIFA predetermined lookup table
// Key: sorted string of 8 qualifying group letters
// Value: [M74, M77, M79, M80, M81, M82, M85, M87] assignments
...
/**
 * Look up the third-place team assignment for a specific R32 match
 *
 * @param qualifyingGroups - Array of 8 group letters that have qualifying 3rd place teams
 * @param matchNumber - The R32 match number (74, 77, 79, 80, 81, 82, 85, 87)
 * @returns The group letter whose 3rd place team plays in this match, or null if not found
 */
export function lookupThirdPlaceAssignment(
  ...
  // Sort the groups alphabetically to create the lookup key
  const key = [...qualifyingGroups].sort().join("");

  const assignments = FIFA_THIRD_PLACE_TABLE[key];
  if (!assignments) return null;

  const matchIndex = THIRD_PLACE_MATCH_ORDER.indexOf(
    matchNumber as 74 | 77 | 79 | 80 | 81 | 82 | 85 | 87,
  );
  if (matchIndex === -1) return null;

  return assignments[matchIndex];
}
