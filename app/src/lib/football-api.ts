import { Match, MatchesResponse, StandingsResponse, TeamsResponse } from '@/types/football'
...
export async function getMatches(): Promise<Match[]> {
  const data = await fetchFromAPI<MatchesResponse>(`/competitions/${COMPETITION_CODE}/matches`)
  return data.matches
}
...
export async function getStandings(): Promise<StandingsResponse> {
  return fetchFromAPI<StandingsResponse>(`/competitions/${COMPETITION_CODE}/standings`)
}
