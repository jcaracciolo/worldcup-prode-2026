import { Match, PointBreakdown, CalculatedStanding, Team } from '@/types/football'
import { Prediction, GroupStandingsOverride } from '@/types/database'
import { getMatchResult, getPredictionResult, isGroupStageMatch } from './football-api'
...
export function calculateGroupStandingsBonusPoints(
  ...
  predictedStandings.forEach((predicted, index) => {
    ...
    if (advancingTeamIds.has(predicted.team.id)) {
      ...
      const actualPosition = actualStandings.findIndex(s => s.team.id === predicted.team.id) + 1
...
export function calculateStandingsFromPredictions(
  ...
  if (overrides.length > 0) {
    overrides.forEach(override => {
      const teamIndex = standings.findIndex(s => s.team.id === override.team_id)
      if (teamIndex !== -1) {
...
export function calculateTotalPoints(
  ...
  groupedMatches.forEach((groupMatchList, groupName) => {
    const groupOverridesForGroup = groupOverrides.filter(o => o.group_name === groupName)
